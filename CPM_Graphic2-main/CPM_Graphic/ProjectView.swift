...
struct GraphicalResultView: View {
    ...
    var body: some View {
        GeometryReader { geo in
            ScrollView {
                VStack () {
                    ForEach(groupedAndSortedActivities().reversed(), id: \.self) { activityGroup in // 그룹된 활동을 역순으로 반복합니다.
                        HStack(spacing: 10) {
                            ForEach(activityGroup.reversed(), id: \.id) { activity in // 활동을 역순으로 반복합니다.
            ...
            .overlay(
                GeometryReader { geometry in
                    ...
                    Color.clear.onAppear {
                        ...
                        activityPositions.updatePosition(for: activity.id, top: bottomCenter, bottom: topCenter)
                    }
                }
